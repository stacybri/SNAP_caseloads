			...
			forval i=1/24 {
				...
				replace d`i'`var'=0
				    if "`var'"=="CertPeriodEarners0103"   {
						replace d`i'`var'=(1-1*l`i'.`var') if yrmo==`yrmo'
						}	
					else if "`var'"=="car_1ex"{
						replace d`i'`var'=-1 if yrmo==`yrmo' & l`i'.car_1notallexempt==1
						}		
					else if "`var'"=="car_allex"{
						replace d`i'`var'=-1 if yrmo==`yrmo' & l`i'.car_allexempt==1
						}							
				   else if "`var'"=="iebt"   {
						replace d`i'`var'=-l`i'.IssuanceEBT if yrmo==`yrmo'
						}	
					else if  "`var'"=="bio"   {
						replace d`i'`var'=1 if yrmo==`yrmo' & l`i'.Biometric==0
						}
					else if  "`var'"=="partban_atleast_nonadult"   {
						replace d`i'`var'=1 if yrmo==`yrmo' & l`i'.partban_atleast_nonadult==0
						}								
					else if "`var'"=="res" {
						replace d`i'`var'=-1 if yrmo==`yrmo' & l`i'.simplify==1
						}
					else if "`var'"=="abawd_final" {
						replace d`i'`var'=(1-1*l`i'.`var') if yrmo==`yrmo' 
						}						
					else if "`var'"=="tranben" {
						replace d`i'`var'=-1 if yrmo==`yrmo' & l`i'.transition==1
						}			
					else if "`var'"=="online" {
						replace d`i'`var'=-1 if yrmo==`yrmo' & l`i'.ApplyOnline==1
						}	
					else if "`var'"=="CatElig" {
						replace d`i'`var'=-1 if yrmo==`yrmo' & l`i'.CashAsstExpCatEligibility==1
						}
					else if "`var'"=="outreachdoll" {
						replace d`i'`var'=(monthdoll_max-l`i'.monthdoll) if yrmo==`yrmo'
						}								
					else {
						replace d`i'`var'=-1 if yrmo==`yrmo' & l`i'.`var'==1
						}
				}
			}
...
sort st_fips se
order se
save ${tdate}_results_${base_mo}_${base_yr}_${end_mo}_${end_yr}/`try1a'_24lags_results_fsp/`try1a'_24lags_fsp_artificial_low, replace
export excel using ${tdate}_results_${base_mo}_${base_yr}_${end_mo}_${end_yr}/`try1a'_24lags_fsp_artificial_low, firstrow(varlabels) replace
keep in 1/2
save ${tdate}_results_${base_mo}_${base_yr}_${end_mo}_${end_yr}/`try1a'_24lags_results_fsp/`try1a'_24lags_fsp_artificial_low_usa, replace
restore
}
...
sort st_fips se
order se
save ${tdate}_results_${base_mo}_${base_yr}_${end_mo}_${end_yr}/`try1a'_12lags_results_fsp/`try1a'_12lags_fsp_artificial_low, replace
export excel using ${tdate}_results_${base_mo}_${base_yr}_${end_mo}_${end_yr}/`try1a'_12lags_fsp_artificial_low, firstrow(varlabels) replace
restore
}
...
sort st_fips se
order se
save ${tdate}_results_${base_mo}_${base_yr}_${end_mo}_${end_yr}/`try1a'_static_results_fsp/`try1a'_static_fsp_artificial_low, replace
export excel using ${tdate}_results_${base_mo}_${base_yr}_${end_mo}_${end_yr}/`try1a'_static_fsp_artificial_low, firstrow(varlabels) replace
restore
}	
